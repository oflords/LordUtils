...
public final class TimeUtil {
    ...
    public static String millisToTimerExtended(long millis) {
        long seconds = millis / 1000L;

        if (seconds > 3600L) {
            return String.format(HOUR_FORMAT, seconds / 3600L, seconds % 3600L / 60L, seconds % 60L);
        } else {
            return String.format(HOUR_FORMAT, seconds / 60L, seconds % 60L, millis % 1000L);
        }
    }
    ...
    public static String millisToRoundedTime(long millis) {
        ...
        if (years > 0) {
            return years + " year" + (years == 1 ? "" : "s") + (negative ? " ago" : "");
        } else if (months > 0) {
            return months + " month" + (months == 1 ? "" : "s") + (negative ? " ago" : "");
        } else if (weeks > 0) {
            return weeks + " week" + (weeks == 1 ? "" : "s") + (negative ? " ago" : "");
        } else if (days > 0) {
            return days + " day" + (days == 1 ? "" : "s") + (negative ? " ago" : "");
        } else if (hours > 0) {
            return hours + " hour" + (hours == 1 ? "" : "s") + (negative ? " ago" : "");
        } else if (minutes > 0) {
            return minutes + " minute" + (minutes == 1 ? "" : "s") + (negative ? " ago" : "");
        } else {
            return seconds + " second" + (seconds == 1 ? "" : "s") + (negative ? " ago" : "");
        }
    }
